...
The site is a static Next.js app in the `V2/` folder of the
...
3. On the configuration screen set **Root Directory** to `V2`.
   Everything else can stay on its defaults — the framework is detected as
   Next.js, the build command is `next build`.
...
- **Root Directory** — `V2`. If the site is later promoted to the repository
  root, change this to `.` in Settings → General.
